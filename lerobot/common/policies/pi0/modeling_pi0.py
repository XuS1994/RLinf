...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.language_tokenizer = AutoTokenizer.from_pretrained("lerobot/model/paligemma-3b-pt-224")
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        samples = self.model.sample_actions(
            images, img_masks, lang_tokens, lang_masks, state
        )
        actions = samples.final_actions
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        ...
        if self.config.lora_config is not None:
            model_to_save.model.paligemma_with_expert.gemma_expert.merge_and_unload()
...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.paligemma_with_expert = PaliGemmaWithExpertModel(paligemma_with_export_config,self.config.lora_config)
        # Projections are float32
        self.state_proj = nn.Linear(self.config.max_state_dim, self.config.proj_width)
        self.action_in_proj = nn.Linear(self.config.max_action_dim, self.config.proj_width)
        self.action_out_proj = nn.Linear(self.config.proj_width, self.config.max_action_dim)
        # add the ppo value projection
        if hasattr(self.config,"adv_method") and self.config.adv_method == "gae":
            self.value_proj = nn.Linear(self.config.proj_width, 1)
    ...
    def forward(
        ...
        # todo: used on the sft, not on the rl
        ...
        u_t = noise - actions
        v_t = self.get_velocity(images,img_masks,lang_tokens,lang_masks,state,x_t,time)
        losses = F.mse_loss(u_t, v_t, reduction="none")
        return losses
    
    def get_velocity(self,images,img_masks,lang_tokens,lang_masks,state,x_t,time):
    ...
    def sample_actions(
        ...
        for idx in range(denoise_steps):
            x_t_mean,x_t_std,value_t = self.sample_mean_var_val(x_t,idx,state,prefix_pad_masks,past_key_values,sample_mode,denoise_steps)
        ...
        return Sample(final_actions=x_0, chains=chains, log_probs=log_probs, value=values)
    
    # todo: to optimize like the forward function without the kv-cache, but requires more memory?
    def get_log_prob_value(
        ...
        # todo: compare the running time with the forward function without the kv-cache, but requires more memory?
        # Compute image and language key value cache
        _, past_key_values = self.paligemma_with_expert.forward(
            attention_mask=prefix_att_2d_masks,
            position_ids=prefix_position_ids,
        ...
        x_t_mean,x_t_std,value_t = self.sample_mean_var_val(chains_pre,denoise_inds,state,prefix_pad_masks,past_key_values,"sde",self.config.num_steps)
        log_probs = self.get_logprob_norm(chains_next,x_t_mean,x_t_std)
        return log_probs, value_t
    

    def sample_mean_var_val(self,x_t,idx,state,prefix_pad_masks,past_key_values,sample_mode,denoise_steps):
        # parameters 
        bsize = state.shape[0]
        device = state.device
        timesteps,sigmas = self.build_parameters(denoise_steps,device) # todo: to reformulate
        # sigma, delta and t_i
        sigma_i = sigmas[idx]
    ...
    # todo: to be checked
    def get_value(self,state,prefix_pad_masks,past_key_values):
        bs = state.shape[0]
        actions_shape = (bs, self.config.n_action_steps, self.config.max_action_dim)
        x_t = torch.zeros(actions_shape,device=state.device)
        timestep = torch.zeros(bs,device=state.device)
        suffix_out = self.get_suffix_out(state,prefix_pad_masks,past_key_values,x_t,timestep)
        suffix_out = torch.mean(suffix_out,dim = 1,keepdim=False)
        value = self.value_proj(suffix_out)
        return value
