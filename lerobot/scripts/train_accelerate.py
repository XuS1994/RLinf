...
def update_policy(
    ...
    policy.train()
    with torch.autocast(device_type=device.type,dtype = torch.float16) if use_amp and accelerator is None else nullcontext():
...
def train(cfg: TrainPipelineConfig, accelerator: Callable = None):
    ...
    if cfg.wandb.enable and cfg.wandb.project:
        ...
        print(colored("Logs will be saved to wandb.", "yellow", attrs=["bold"]))
    else:
        wandb_logger = None
...
if __name__ == "__main__":
    ...
    if is_launched_with_accelerate():
        print("is_launched_with_accelerate")
        import accelerate
        ...
        print("not is_launched_with_accelerate")
        train()